...
def addTaskToBoard(boardObj, columnName, taskObj): 
   ...
   return 0 
       #print("IN COL FOR. CURRENT NAME: ", col.columnTitle, " COLUMNLIST SIZE: ", len(boardObj.columnList), " TASKSIZE: ", len(col
.taskList)) 
...
def getCertainUser(dbObj, userToFind): 
   ...
   if (newUser.name == "default user name"): 
       ...
       print("Since you are a new user, you are not a part of any boards. You can create a new one with <new board>, or wait to be 
added to one.") 
       return newUser 
           ...
           for taskIndex in range(0, len(boardObj.columnList[colIndex].taskList)): 
               if (boardObj.columnList[colIndex].taskList[taskIndex].name == taskToEditName): 
                   ...
                   if (returnVal == 1): 
                       ...
                       #popped = boardObj.columnList[colIndex].taskList[taskIndex].pop(boardObj.columnList[colIndex].taskList[taskI
ndex]) 
...
def updateDbBoard(dbObj, boardObj): 
   ...
   for col in boardObj.columnList: 
       for task in col.taskList: 
           ...
           dbObj.child("Boards").child(boardObj.key).child("Tasks").child(col.columnTitle).child(task.key).update({"Name" : task.na
me, 
                                                                                                                    "DueDate" : tas
k.dueDate, 
                                                                                                                    "LatestAction" 
...
task.description}) 
           ...
           for owners in task.ownerList: 
               print("OWNER: ", owners) 
               dbObj.child("Boards").child(boardObj.key).child("Tasks").child(col.columnTitle).child(task.key).child("Owners").upda
te({owners : True}) 
